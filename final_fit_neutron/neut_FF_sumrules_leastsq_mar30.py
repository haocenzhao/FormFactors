...
rE2 = -0.1161 #fm
drE2 = 0.0001 #fm divided by 10 from the original value 0.04 fm
rM = 0.864   #fm^2
...
# radius contraint with the exisiting results
    ...
    if iter %5 == 0:
        print(iter, 'chi2gn=%f, chi2Ngn=%f,chi2R=%f, chi2gncoef=%f, dof=%d, %d, %d, %d'%( chi2gn, chi2Ngn,chi2rad, chi2gncoef, count1, count2, count3,count4))
...
if kmax<=6:
    ...
    prv_file= sys.argv[6]
    prv_file= prv_file.replace('mar30_world_','sep20_world_')
    prv_file= prv_file.replace('mar30_world_','sep20_world_')
    if method ==1:
        ...
        prv_file = prv_file.replace('_t0zero','_t0opt')
    elif method ==3:
...
print('Total no. data points:', numGN)
...
print(ndof, nmax, numGN,kGN,kNorm, file=of)
print(redchi2, chi2, chi2Ngn, chi2gncoef, chi2gnfake, chi2rad, file=of)
