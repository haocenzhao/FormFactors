...
def fitfuncsumrules(params):
   ...
   # fake GEp point for high-Q2 contraint
    ...
    if iter %500 == 0:
        print(iter, chi2xsMainz+chi2csyst+chi2NMainz+chi2xsworld+chi2Nworld+chi2pol+chi2gefake+chi2gmfake+chi2gefakeHQ+chi2gmfakeHQ+chi2gecoef+chi2gmcoef)
        print(chi2xsMainz, chi2xsworld, chi2pol, chi2gefake, chi2gmfake, chi2gefakeHQ, chi2gmfakeHQ)
...
if kmax>=6 and Q2max>0 and method==1:
    ...
    prv_file= output_file 
    prv_file = prv_file.replace('mar30_', '../nov07_')
    prv_file = prv_file.replace('mar30_', 'nov07_')
...
start = time.clock() # time for diagnostics
...
chi2 = chi2xsMainz+chi2csyst+chi2NMainz+chi2xsworld+chi2Nworld+chi2pol+chi2gefake+chi2gmfake+chi2gefakeHQ+chi2gmfakeHQ+chi2gecoef+chi2gmcoef+chi2erad+chi2mrad
...
print('Chi^2 Mainz xsec, world xsec, pol, gefake, gmfake:', chi2xsMainz, chi2xsworld, chi2pol, chi2gefakeHQ, chi2gmfakeHQ)
...
print('Total no. data points:', numMainz+numworld+numpol+numfake+numfakeHQ)
print('No. data points Mainz, world, pol, fake, fakeHQ:', numMainz, numworld, numpol, numfake, numfakeHQ)
...
print(ndof, numMainz+numworld+numpol+numfake+numfakeHQ, numMainz, numworld, numpol, numfake, nmax, nmax, kslope, kMainz, kworld, numfakeHQ, file=of)
print(redchi2, chi2, chi2xsMainz, chi2xsworld, chi2pol, chi2gefake, chi2gmfake, chi2gecoef, chi2gmcoef, chi2csyst, chi2NMainz, chi2Nworld, chi2gefakeHQ, chi2gmfakeHQ, chi2erad,chi2mrad, file=of)
...
for i in range(2*nmax):
    ...
    for j in range(2*nmax):
        covrow.append(cov[i][j]*redchi2) # need to multiply cov by reduced chi^2 to get true approximation to Hessian
