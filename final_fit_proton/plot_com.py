...
folder = './mar30_all_FF/z'+str(kmax) # folder containing central fits
...
Q2str1 = str(Q2max1) 
mod1 = 't0fix7'
folder = './mar30_%s_FF/z'%comp+str(kmax1) # folder containing central fits
...
#########################{{{
def loadGEGM(Q2_max):
    fitlines = open('../data/GEGM_John.dat', 'r').readlines()
    N=len(fitlines)-1 + 5
    Q2=np.zeros(N, dtype=float)
    ...
    lines1= open('../data/prot_GEGM_fake.dat').readlines()
    ...
    lines2= open('../data/prot_GEGM_fakeHQ_sep20.dat').readlines()
    ...
    Q2[N-5]=float(values1[0])
    GEdip[N-5]=float(values1[1])
    dGEdip[N-5]=float(values1[2])
    GMdip[N-5]=float(values1[3])
    dGMdip[N-5]=float(values1[4])
    #GEGMdip[N-5]=float(values1[5])
    #dGEGMdip[N-5]=float(values1[6])  
    GEGMdip[N-5]= -1000.0
    dGEGMdip[N-5]= 0.0
...
def loadPolGEGM(Q2_max):
    ...
    dR_Err=[]
    #idat=[]
    lines= open('../data/World_GeGm_Polarization.dat').readlines()
...
########
axes[3,0].plot(Q2, GE_fit_rat/GE_fit_rat_n, 'k--', color='black',  label='GEn-fit(blue)/GEn-fit(red)')
axes[3,0].plot(Q2, GM_fit_rat/GM_fit_rat_n, 'k-.', color='m',  label='GMn-fit(blue)/GMn-fit(red)')
...
axes[3,0].set_ylim(0.99,1.02)
axes[3,0].set_xscale('log')
axes[3,0].grid()

axes[3,1].plot(Q2, (GE_pos_rat-GE_fit_rat), 'k--', color='blue',  label='$\delta G_E^n/G_D$ (%s)'%com1)
axes[3,1].plot(Q2, (GM_pos_rat-GM_fit_rat), 'k-.', color='blue',  label='$\delta G_M^n/\mu G_D (%s)$'%com1)

axes[3,1].plot(Q2, (GE_pos_rat_n-GE_fit_rat_n), 'k--', color='r',  label='$\delta G_E^n/G_D (%s)$'%com2)
axes[3,1].plot(Q2, (GM_pos_rat_n-GM_fit_rat_n), 'k-.', color='r',  label='$\delta G_M^n/\mu G_D (%s)$'%com2)


axes[3,1].set_xlim(2e-3, Q2plot1)
axes[3,1].set_ylabel('$\delta G^n/G_D$') # label, y-axis
#axes[3,1].set_ylim(0.5,1.5)
...
i0+=1.5
axes[1,0].text(x0, y0-0.02*i0,  r'$<r^{E}_{%s}>=%5.4f\pm %5.4f$'%(comp,erad_n,derad_n),
              fontproperties=font_prop1, color='r')
i0+=2
axes[1,0].text(x0, y0-0.02*i0,  r'$<r^{M}_{all}>=%5.4f\pm %5.4f$'%(mrad, dmrad),
              fontproperties=font_prop1, color='blue')
i0+=1.5
axes[1,0].text(x0, y0-0.02*i0,  r'$<r^{M}_{%s}>=%5.4f\pm %5.4f$'%(comp,mrad_n, dmrad_n),
              fontproperties=font_prop1, color='r')
...
axes[0,0].text(0.05, 1.05, r'world+Mainz+Pol Data Set w/ extra TPE', fontproperties=font_prop1,color='blue')
axes[0,0].text(0.05, 1.02, r'%s Data Set w/ extra TPE'%comp, fontproperties=font_prop1,color='red')


plt.savefig('Com_all_%s.pdf'%comp)
plt.savefig('Com_all_%s.png'%comp)
